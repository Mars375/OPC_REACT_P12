...
const BarChart = ({ data }: { data: SessionProps[] }) => {
	...
	useEffect(() => {
		...
		const margin = { top: 50, left: 50, right: 40, bottom: 30 };
		...
		const width = Math.min(window.innerWidth * 0.7, maxWidth);
		const height = Math.min(window.innerHeight * 0.35, maxHeight);
		...
		barGroups
			...
			.attr("y", window.innerWidth > 1024 ? margin.bottom + 15 : margin.bottom)
			.attr("width", 70)
			.attr(
				"height",
